...
class SorController extends Controller
{
    ...
    public function store(Request $request){
        ...
        if ($request->hasFile('file')) {
            ...
            if (!in_array($mimeType, $allowedMimeTypes)) {
                ...
                $request_id = $nik.'.'.$request->project_id.'.'.$request->route_id.'.'.$request->segment_id.'.'.$request->section_id.'.'.$currentDateTime. '.' . $randomNumber;
                $app_path = storage_path('app/public/sor_request/'.$nik.'/'.$project_id.'/'.$route_id.'/'.$segment_id.'/'.$section_id.'/'.$request_id);
    ...
    public function process($request_id){
        ...
        $file_path = storage_path('app/public/sor_request/'.$sor_request->requestor.'/'.$sor_request->project_id.'/'.$sor_request->route_id.'/'.$sor_request->segment_id.'/'.$sor_request->section_id.'/'.$sor_request->request_id.'/'.$sor_request->request_id);
        
        if (file_exists($file_path . '.zip')) {
            $zip = new ZipArchive();
            if ($zip->open($file_path . '.zip') === TRUE) {
                if(isset($sor_request->sub_section_id)){
                    $folderPath = storage_path('app/public/sor_request/'.$sor_request->requestor . '/' . $sor_request->project_id . '/' . $sor_request->segment_id . '/' . $sor_request->section_id.'/'.$sor_request->sub_section_id.'/'. $sor_request->request_id);
                }else{
                    $folderPath = storage_path('app/public/sor_request/' . $sor_request->requestor . '/' . $sor_request->project_id . '/' . $sor_request->segment_id . '/' . $sor_request->section_id . '/' . $sor_request->request_id);
                }
                ...
                foreach ($files as $file) {
                    ...
                    if(isset($sor_request->sub_section_id)){
                        $path = storage_path('app/public/'.$sor_request->project_id.'/'.$sor_request->segment_id.'/'.$sor_request->section_id.'/'.$sor_request->sub_section_id.'/'.$fileName.'.sor');
                    }else{
                        $path = storage_path('app/public/'.$sor_request->project_id.'/'.$sor_request->segment_id.'/'.$sor_request->section_id.'/'.$fileName.'.sor'); 
                    }
            ...
            return redirect()->back()->with('error', 'File not found.'.storage_path('app/public/sor_request/'.$sor_request->requestor.'/'.$sor_request->project_id.'/'.$sor_request->route_id.'/'.$sor_request->segment_id.'/'.$sor_request->section_id.'/'.$sor_request->request_id.'/'.$sor_request->request_id));
            // return redirect()->back()->with('error', 'File not found.'.$sor_request->project_id);
        }
    } 
    // C:\Laravel Dev\cms_v2_development\storage\app\public\sor_request\lapangan\001\2\01\01\lapangan.001.2.01.01.20250208215341.823054\lapangan.001.2.01.01.20250208215341.823054.zip
    // C:\Laravel Dev\cms_v2_development\storage\app/public/sor_request/lapangan/1/2/1/1/lapangan.001.2.01.01.20250208215341.823054/lapangan.001.2.01.01.20250208215341.823054
    public function approval($request_id, $status){
        set_time_limit(0);

        $sor_request = DB::table('sor_request')->where('request_id', $request_id)->first();
        $section = DB::table('section')->where('section_id', $sor_request->section_id)->get()->first();
        $sub_section_id = '-';
        $cores = DB::table('core')->where('project_id', $sor_request->project_id)->where('segment_id', $sor_request->segment_id)->where('section_id', $sor_request->section_id)->get();
        $path = storage_path('app/public/sor_request/' . $sor_request->requestor . '/' . $sor_request->project_id . '/' . $sor_request->segment_id . '/' . $sor_request->section_id . '/' . $sor_request->request_id);
        $live_folder = storage_path('app/public/' . $sor_request->project_id . '/' . $sor_request->segment_id . '/' . $sor_request->section_id);
        ...
        if($status == 'APPROVE'){
            ...
            if($sor_request->sub_section_id == null){
                return redirect()
                ->route('section.show', ['project_id'=>$sor_request->project_id, 'project_type'=>$section->cable_category, 'segment_id'=> $sor_request->segment_id, 'section_id' => $sor_request->section_id])
                ->with('success', 'Request is successfully approved !');   
            }else{
                return redirect()
                ->route('sub_section.show', ['project_id'=>$sor_request->project_id, 'project_type'=>$section->cable_category, 'segment_id'=> $sor_request->segment_id, 'section_id' => $sor_request->section_id, 'sub_section_id'=> $sub_section_id])
                ->with('success', 'Request is successfully approved !');   
            }
