...
@section('breadcrumb')
    @php
        $project = DB::table('project')->where('project_id', $segment->project_id)->get()->first();
    @endphp
    <div class="card bg-light-info shadow-none position-relative overflow-hidden">
        <div class="card-body px-4 py-5">
            <div class="row align-items-center">
                <div class="col-9">
                    <h3 class="fw-semibold" style="font-size: 2rem;">Edit Segment : {{$segment->segment_name}}</h3>
                    <hr>
                    <nav aria-label="breadcrumb">
                        <ol class="breadcrumb">
                            ...
                            <li class="breadcrumb-item">
                                <a class="text-decoration-none" href="{{route('project.show', ['project_id'=> $segment->project_id])}}">{{$project->project_name}}</a>
                            </li>
    ...
    <div class="row">
        <div class="col-lg-12 align-items-stretch">
            <div class="card w-100" style="padding:2rem;">
                <form action="{{route('segment.update')}}" method="POST">
                    {{ csrf_field() }}
                    <input type="hidden" readonly name="segment_id" type="text" value="{{$segment->segment_id}}">
                    <input type="hidden" readonly name="project_id" type="text" value="{{$segment->project_id}}">
                    <label>Segment Name</label>
                    <input type="text" class="form-control mb-2" name="segment_name" value="{{$segment->segment_name}}">
                    <hr>
